  ...
  return (
    <div className="bg-neutral-900 min-h-screen py-14 px-4">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl md:text-4xl font-bold text-yellow-400 mb-10 text-center">コラム</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {columns.map((col) => (
            <Link key={col.slug} href={`/column/${col.slug}`} className="group">
              <div className="bg-neutral-800 rounded-2xl shadow-lg overflow-hidden hover:scale-105 hover:shadow-2xl transition transform duration-200">
                ...
                <div className="p-5">
                  <h2 className="font-bold text-lg text-yellow-300 mb-2 group-hover:text-yellow-400 transition">{col.title}</h2>
                  <p className="text-gray-200 text-sm">{col.excerpt}</p>
                </div>
              </div>
            </Link>
          ))}
